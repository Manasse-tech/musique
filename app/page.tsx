...
export default function Melodix() {
  ...
  const [sleepTimer, setSleepTimer] = useState("Désactivé")
  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      ...
      {section === "settings" ? <Settings eqPreset={eqPreset} setEqPreset={setEqPreset} sleepTimer={sleepTimer} setSleepTimer={setSleepTimer} /> : section === "home" ? <Home audio={audio} videos={videos} play={play} importFiles={importFiles} /> : <MediaSection section={section} items={shown} play={play} favorites={favorites} toggleFavorite={toggleFavorite} />}
    </main>
    {active && <Player item={active} playing={playing} setPlaying={setPlaying} audioRef={audioRef} videoRef={videoRef} showQueue={showQueue} setShowQueue={setShowQueue} onClose={() => { setPlaying(false); setActive(null) }} />}
...
function Player({ item, playing, setPlaying, audioRef, videoRef, showQueue, setShowQueue, onClose }: { item: MediaItem; playing: boolean; setPlaying: (value: boolean) => void; audioRef: RefObject<HTMLAudioElement | null>; videoRef: RefObject<HTMLVideoElement | null>; showQueue: boolean; setShowQueue: (value: boolean) => void; onClose: () => void }) { useEffect(() => { const media = item.kind === "audio" ? audioRef.current : videoRef.current; if (!media) return; if (playing) void media.play().catch(() => setPlaying(false)); else media.pause() }, [item, playing, audioRef, videoRef, setPlaying]); return <div className={`player-dock ${item.kind === "video" ? "video-player" : ""}`}>{item.kind === "video" ? <video ref={videoRef} src={item.url} controls playsInline /> : <audio ref={audioRef} src={item.url} controls /> }<div className="player-meta"><span className="mini-art">{item.kind === "video" ? "▶" : "♫"}</span><div><strong>{item.name}</strong><small>{item.kind === "video" ? "Vidéo locale" : "Audio local"}</small></div><button className="player-action" onClick={() => setPlaying(!playing)} aria-label={playing ? "Pause" : "Lecture"}>{playing ? "Ⅱ" : "▶"}</button><button className="player-action" onClick={() => setShowQueue(!showQueue)} aria-label="Afficher la file d'attente">☷</button><button className="player-action" onClick={onClose} aria-label="Fermer le lecteur">×</button></div></div> }
function Settings({ eqPreset, setEqPreset, sleepTimer, setSleepTimer }: { eqPreset: string; setEqPreset: (value: string) => void; sleepTimer: string; setSleepTimer: (value: string) => void }) { return <div className="content-wrap"><div className="page-header"><div><p className="eyebrow">PRÉFÉRENCES</p><h1>Réglages</h1><p className="muted">Personnalisez votre écoute, gratuitement et sans publicité.</p></div></div><div className="settings-grid"><div className="settings-card"><h2>Audio studio</h2><p>Choisissez un profil sonore adapté à chaque moment.</p><label className="setting-line">Égaliseur<select value={eqPreset} onChange={(event) => setEqPreset(event.target.value)}>{["Flat","Classique","Dance","Bass boost","Jazz","Rock","Hip-hop"].map((preset) => <option key={preset}>{preset}</option>)}</select></label><label className="setting-line">Minuteur<select value={sleepTimer} onChange={(event) => setSleepTimer(event.target.value)}>{["Désactivé","15 minutes","30 minutes","60 minutes"].map((timer) => <option key={timer}>{timer}</option>)}</select></label></div><div className="settings-card"><h2>Lecture locale</h2><p>Audio et vidéo restent sur votre appareil. La lecture reprend via les contrôles natifs du navigateur.</p><div className="feature-row"><span>Lecture en arrière-plan</span><b>Selon le navigateur</b></div><div className="feature-row"><span>Formats audio</span><b>MP3 · WAV · FLAC · M4A</b></div><div className="feature-row"><span>Formats vidéo</span><b>MP4 · WEBM · MOV · MKV</b></div></div></div></div> }
